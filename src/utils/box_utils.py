...
def refine_match(threshold, bbox_gt, priors, variances, labels, loc_t, conf_t, idx, arm_loc=None):
    """Match each arm bbox with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
        threshold: (float) The overlap threshold used when mathing boxes.
        bbox_gt: (tensor) Ground truth boxes, Shape: [num_obj,4].
    ...
    # jaccard index
    if arm_loc is None:
        overlaps = jaccard(bbox_gt, point_form(priors))
    else:
        decode_arm = decode(arm_loc, priors=priors, variances=variances)
        overlaps = jaccard(bbox_gt, decode_arm)
    # (Bipartite Matching)
    ...
    # the value of best_prior_idx is the index of __overlap
...
def nms(boxes, scores, overlap=0.5, top_k=2000):
...
def nms_(boxes,scores, threshold=0.7,topk=200,mode='Union'):
    pick = []
    count = 0
    if boxes.size()==0:
        return pick,count
